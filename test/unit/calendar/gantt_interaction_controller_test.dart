...
void main() {
  ...
  ProviderContainer makeContainer() {
    final container = ProviderContainer(
      overrides: [
        ...
        clockProvider.overrideWith((ref) => () => DateTime.utc(2026, 6, 7)),
      ],
    );
  ...
  test('MoveDrag shifts both start and due dates by the delta', () async {
    ...
    await controller(container).apply(
      const MoveDrag(taskId: 't1', delta: Duration(days: 2)),
    );
  ...
  test('valid ResizeDrag(end) extends the due date', () async {
    ...
    expect(
      repo.items.first.dueDate,
      DateTime.utc(2026, 6, 15),
    );
  });

  test('invalid ResizeDrag (due before start) does NOT write to the repo',
      () async {
    final original = Task(
      id: 't1',
      title: 'Keep me',
  ...
  test('invalid ResizeDrag (start after due) does NOT write to the repo',
      () async {
    repo.seed([
      Task(
        id: 't1',
